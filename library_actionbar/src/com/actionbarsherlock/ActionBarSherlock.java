...
public abstract class ActionBarSherlock {
    ...
    public static ActionBarSherlock wrap(Activity activity, int flags) {
        ...
        for (Implementation key : impls.keySet()) {
            ...
            if (key.dpi() == DisplayMetrics.DENSITY_MEDIUM) {
                hasQualfier = true;
                break;
            }
        }
        if (hasQualfier) {
            final boolean isTvDpi = activity.getResources().getDisplayMetrics().densityDpi == DisplayMetrics.DENSITY_MEDIUM;
            for (Iterator<Implementation> keys = impls.keySet().iterator(); keys.hasNext(); ) {
                int keyDpi = keys.next().dpi();
                if ((isTvDpi && keyDpi != DisplayMetrics.DENSITY_MEDIUM)
                        || (!isTvDpi && keyDpi == DisplayMetrics.DENSITY_MEDIUM)) {
                    keys.remove();
                }
            }
        }
