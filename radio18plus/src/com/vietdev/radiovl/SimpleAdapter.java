...
public class SimpleAdapter extends BaseAdapter implements Filterable {
	...
	private int[] mToSeparator;
	private String[] mFromSeparator;
	...
	public static class ViewVisibility {
		...
		public ViewVisibility(int gone) {
			viewVisibility = gone;
		}
	...
	public SimpleAdapter(Context context, List<? extends Map<String, ?>> data, int resource, String[] from, int[] to, int separatorResource, String[] sfrom, int[] sto) {
		mData = data;
		mResource = mDropDownResource = resource;
		mFrom = from;
		mTo = to;
		...
		this.mSeparatorResource = separatorResource;
		this.mToSeparator = sto;
		this.mFromSeparator = sfrom;
	}
	 ...
	 * @param from
	 * @param to
	 * @param ids View id need to reset LayoutParams.
	 * @param llparams The LayoutParams apply for specific Views in list ids
	 */
	public SimpleAdapter(Context context, List<? extends Map<String, ?>> data, int resource, String[] from, int[] to, int[] ids, LinearLayout.LayoutParams[] llparams) {
		mData = data;
		mResource = mDropDownResource = resource;
		mFrom = from;
		mTo = to;
		mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		this.ids = ids;
		this.layoutParams = llparams;
	}

	private final String ITEM_TYPE = "item_type";
	private final String TYPE_SEPARATOR = "separator";

	public HashMap<String, Object> getSeparator() {
		HashMap<String, Object> mapSeparator = new HashMap<String, Object>();
		mapSeparator.put(ITEM_TYPE, TYPE_SEPARATOR);
	...
	private View createViewFromResource(int position, View convertView, ViewGroup parent, int resource) {
		...
		if (mData.get(position).get(ITEM_TYPE) != null && mData.get(position).get(ITEM_TYPE).toString().equalsIgnoreCase(TYPE_SEPARATOR)) {
			System.out.println("Type> separator, text>"+mData.get(position).get("separaterText"));
			View v;
			v = mInflater.inflate(mSeparatorResource, parent, false);
			v.setTag("devider");
			bindViewSeparator(position, v);
			return v;
		}
		View v;
		...
		if (convertView == null || convertView.getTag() != null) {
			...
			for (int i = 0; ids != null && i < ids.length; i++) {
				int id = ids[i];
				View adjustView = v.findViewById(id);
				adjustView.setLayoutParams(layoutParams[i]);
			}
	...
	private void bindViewSeparator(final int position, View view) {
		...
		final String[] from = mFromSeparator;
		final int[] to = mToSeparator;
		...
		for (int i = 0; i < count; i++) {
			...
			if (v != null) {
				...
				if (!bound) {
					if (data instanceof ViewVisibility) {
						ViewVisibility visible = (ViewVisibility) data;
						v.setVisibility(visible.isVisible() ? View.VISIBLE : View.GONE);
					} else if (data instanceof OnClickListener) {
						OnClickListener onclick = (OnClickListener) data;
